...
def load_db(df):
    ...
    for _, row in df.iterrows():
        source = db.query(models.Source).filter_by(name=row['source']).first()
        if not source:
            source = models.Source(name=row['source'])
            db.add(source); db.commit(); db.refresh(source)

        asset = db.query(models.Asset).filter_by(ticker=row['ticker']).first()
        if not asset:
            asset = models.Asset(ticker=row['ticker'], currency=row['currency'], 
                                 asset_type=row['type'], source_id=source.id)
            db.add(asset); db.commit(); db.refresh(asset)

        db.add(models.Quote(asset_id=asset.id, price=row['price']))
    db.commit()
    db.close()
